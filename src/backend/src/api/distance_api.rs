...
pub fn car_stats() -> CarTravelStats {
    STATE.with_borrow(|state| {
        let bookings: Vec<RentalTransaction> = state
            .cars
            .iter()
            .map(|f| f.1.bookings)
            .flatten()
            .map(|f| f.1.without_customer())
            .collect();
        let total_revenue: f64 = bookings.iter().map(|f| f.total_amount).sum();
        let total_distance_travelled: f64 =
            state.car_travel_details.iter().map(|f| f.1.distance).sum();
        CarTravelStats {
            total_revenue,
...
pub fn add_distance_travelled(car_id: u64, distance: f64, notes: Option<String>) {
    STATE.with(|state| {
        ...
        state.car_travel_details.insert(
            car_id,
            crate::DistanceTravelled {
                car_id,
                distance,
                current_timestamp,
                caller,
